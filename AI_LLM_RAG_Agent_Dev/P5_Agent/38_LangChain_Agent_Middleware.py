...
def init_chat_model() -> ChatOpenAI:
    ...
    if not api_key:
        raise ValueError(
            "未找到 OPENAI_API_KEY 或 API_KEY 环境变量，请先在 .env 或系统环境中配置后再运行。"
        )

    # LangChain 的 OPENAI_API_KEY 封装会自动从环境变量中读取 key，
    # 这里设置一份到 DASHSCOPE_API_KEY，确保兼容性。
    os.environ["OPENAI_API_KEY"] = api_key

    chat = ChatOpenAI(model="glm-5.1")
    return chat
